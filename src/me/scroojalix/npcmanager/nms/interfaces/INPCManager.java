...
public abstract class INPCManager {
	...
	public void createNPC(String name, Location loc) {
		...
		NPCData data = new NPCData(name, displayName, null, loc, 60, true);
		restoreNPC(data);
	}
	...
	public void saveYAMLNPCs(FileManager file) {
		...
		for (Map.Entry<String, NPCData> entry : NPCs.entrySet()) {
			if (entry.getValue().getLoc() != null) {
				data.set("npc."+entry.getKey()+".world", entry.getValue().getLoc().getWorld().getName());
				data.set("npc."+entry.getKey()+".x", entry.getValue().getLoc().getX());
				data.set("npc."+entry.getKey()+".y", entry.getValue().getLoc().getY());
				data.set("npc."+entry.getKey()+".z", entry.getValue().getLoc().getZ());
				data.set("npc."+entry.getKey()+".yaw", entry.getValue().getLoc().getYaw());
				data.set("npc."+entry.getKey()+".pitch", entry.getValue().getLoc().getPitch());
				if (entry.getValue().getDisplayName() != null) data.set("npc."+entry.getKey()+".displayName", entry.getValue().getDisplayName());
				if (entry.getValue().getSubtitle() != null) data.set("npc."+entry.getKey()+".subtitle", entry.getValue().getSubtitle());
				if (entry.getValue().getSkin() != null) data.set("npc."+entry.getKey()+".skin", entry.getValue().getSkin());
				else data.set("npc."+entry.getKey()+".id", entry.getValue().getUUID());
				if (entry.getValue().getInteractEvent() != null) data.set("npc."+entry.getKey()+".interactEvent", entry.getValue().getInteractEvent().getInteractionName().replace(" ", "-"));
				data.set("npc."+entry.getKey()+".range", entry.getValue().getRange());
				data.set("npc."+entry.getKey()+".hasHeadRotation", entry.getValue().hasHeadRotation());
			} else {
	...
	public void restoreYAMLNPCs(FileManager file) {
		...
		data.getConfigurationSection("npc").getKeys(false).forEach(current -> {
			...
			if (Bukkit.getWorld(worldName) != null) {
				...
				NPCData npcdata = new NPCData(current, displayName, null, loc, range, hasHeadRotation);
				npcdata.setSubtitle(subtitle);
				if (data.isSet("npc."+current+".skin")) npcdata.setSkin(data.getString("npc."+current+".skin"));
