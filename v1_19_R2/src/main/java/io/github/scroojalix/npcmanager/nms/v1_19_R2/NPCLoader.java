...
public class NPCLoader extends INPCLoader implements Runnable {
	...
	protected void generatePackets() {
		...
		packets.add(new ClientboundPlayerInfoUpdatePacket(ClientboundPlayerInfoUpdatePacket.Action.ADD_PLAYER, npc));
		packets.add(new ClientboundAddPlayerPacket(npc));
		packets.add(new ClientboundSetEntityDataPacket(npc.getId(), npc.getEntityData().packDirty()));
		...
		if (data.getNameHolo() != null) {
			...
			packets.add(new ClientboundSetEntityDataPacket(holo.getId(), holo.getEntityData().packDirty()));
		}
